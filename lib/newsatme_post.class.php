...
class NewsAtMe_Post {
  ...
  private function getCategoryLikeTerms() {
    return self::getTaxonomyNames( wp_get_object_terms($this->id, $this->taxonomies['hierarchical']) ) ;
  }

  private function getTagLikeTerms() {
    return self::getTaxonomyNames( wp_get_object_terms($this->id, $this->taxonomies['non-hierarchical']) ) ;
  }

  /**
   * This function was introduced a suspect of a bug in WP 4.7
   *
   * function:
   *
   *    wp_get_object_terms($this->id, $this->taxonomies['non-hierarchical'], array('fields' => 'names' ) )
   *
   * no longer returns an array of taxonomy names. It returns an empty array.
   * Checking for property to exist may be required for compatibility with pre 4.7 versions.
   *
   * @param $terms
   *
   * @return array
   */
  private function getTaxonomyNames( $terms ) {
